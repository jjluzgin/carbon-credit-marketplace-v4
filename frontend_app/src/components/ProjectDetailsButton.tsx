...
import { Calendar, Scale, Leaf, User, Info, Check, BarChart3, File, FileText, Variable } from "lucide-react";
...
const ProjectDetailsButton: React.FC<DetailsButtonProps> = ({ tokenId, account, size = "sm", variant }) => {
  ...
  const handleGetDetails = async () => {
    ...
    try {
      const [status, , owner, auditor, authenticationDate, carbonRemoved, creditsIssued, ipfsCID] = await readContract({
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button size={size} variant={variant} onClick={handleGetDetails}>
          Details
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>Verification ID: {projectData?.verificationId}</DialogDescription>
        </DialogHeader>
                ...
                {projectData && (
                  <>
                    ...
                    <Button
                      ...
                      size="sm"
                      className="gap-2"
                      onClick={() => window.open(`https://ipfs.io/ipfs/${projectData.ipfsCID}/`, "_blank")}>
                      View Data
                    </Button>
                  </>
                )}
